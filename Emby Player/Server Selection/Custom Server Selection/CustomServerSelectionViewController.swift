...
class CustomServerSelectionViewController: UIViewController {
    ...
    lazy var serverIPField: UITextField     = self.createTextField(placeholder: Strings.ipAddressPlaceholder,
                                                                   text: "", keybordType: .URL)
    lazy var serverPortField: UITextField   = self.createTextField(placeholder: Strings.portPlaceholder,
                                                                   text: "8096",
                                                                   keybordType: .numberPad)
    lazy var errorTextLabel: UILabel        = self.createErrorTextLabel()
    lazy var connectButton: UIButton        = self.createButton(title: Strings.connectTitle,
                                                                color: .green,
    ...
    private func getServerConnection() throws -> ServerConnection {
        ...
        guard let ipAddress = serverIPField.text,
            !ipAddress.isEmpty              else { throw Errors.missingIP }
        guard let portString = serverPortField.text,
            let port = Int(portString)      else { throw Errors.missingPort }
